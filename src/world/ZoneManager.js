...
export class ZoneManager {
  constructor(scene, renderer, hemiLight, sky, vfx) {
    ...
    this.zones = [
      { 
        name: 'LA 2049', type: 'LA_2049', pos: new THREE.Vector3(0, 0, 0), 
      ...
      { 
        name: 'WALLACE HQ', type: 'WALLACE_HQ', pos: new THREE.Vector3(500, 0, -500), 
    ...
    this.ambientSounds = {}; // Cache for Howl instances
  ...
  updateAtmosphere(playerPos) {
    ...
    if (closest.name !== this.currentZoneData.name) {
      this.crossfadeAmbient(this.currentZoneData.name, closest.name);
  ...
  crossfadeAmbient(oldZone, newZone) {
    ...
    if (oldSound) oldSound.fade(oldSound.volume(), 0, 2000);
    if (newSound) {
      if (!newSound.playing()) newSound.play();
      newSound.fade(0, 0.5, 2000);
    }
  }
